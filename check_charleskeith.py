"""
CHARLES & KEITH 상품 페이지(들)가 '구매 가능(품절 아님)' 상태로 바뀌면 알림.

여러 상품(사이즈/색상별 URL)을 동시에 감시할 수 있다
(PRODUCT_URLS 를 쉼표로 구분해서 여러 개 지정).

동작 방식
1. 각 상품 페이지 HTML을 그대로 요청한다 (서버 렌더링이라 JS 실행 없이도
   품절 여부가 HTML에 그대로 포함되어 있음을 확인했다).
2. 'js-notify-me-btn' 클래스 문자열이 있으면 품절, 없으면 구매 가능으로 판단한다.
3. 직전 상태(charleskeith_state.json)와 비교해서 품절 -> 구매가능 으로 바뀐 상품이
   있으면 알림을 보낸다.
...
PRODUCT_URLS = [
    ...
    for u in os.environ.get(
        "CK_PRODUCT_URLS",
        "https://www.charleskeith.com/kr/CK2-20151673_SFT.PK.html",
    ).split(",")
    if u.strip()
]

STATE_FILE = Path(os.environ.get("CK_STATE_FILE", "charleskeith_state.json"))

SOLDOUT_MARKER = "js-notify-me-btn"
...
def fetch_is_available(url: str) -> bool:
    ...
    html = resp.text
    is_soldout = SOLDOUT_MARKER in html
...
def main() -> int:
    ...
    for url in PRODUCT_URLS:
        ...
        if became_available:
            ...
            message = f"[CHARLES & KEITH] 상품이 구매 가능해졌습니다!\n\n{url}"
